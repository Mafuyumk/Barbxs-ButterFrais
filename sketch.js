...
function setup() {
  ...
  ghost = createSprite(200,200,80,80);
  ghost.scale = 0.6;
  ghost.addImage("ghost", ghostImg);
  ghost.debug = true
  ghost.setCollider("rectangle", -5,5,140,155);
}
...
//escreva um código para fazer invisibleBlockGroup (grupo de bloco invisível) colidir com o fantasma, destruir o fantasma e mudar o estado do jogo para end.
    ...
    if(invisibleBlockGroup.isTouching(ghost) || ghost.y > 600){
      ghost.destroy(invisibleBlockGroup);
      gameState = "end";
    }
...
function spawnDoors()
 {
  ...
  if (frameCount % 240 === 0) {
    ...
    var invisibleBlock = createSprite(x,25);
